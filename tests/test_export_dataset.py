...
def _make_kline_rows(n: int = 5) -> list[dict]:
    ...
    for i in range(n):
        ...
        rows.append({
            ...
            "open_dt_utc": datetime.fromtimestamp(ts / 1000, timezone.utc).replace(tzinfo=None).isoformat(),
            "close_dt_utc": datetime.fromtimestamp((ts + 3600_000 - 1) / 1000, timezone.utc).replace(tzinfo=None).isoformat(),
